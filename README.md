...
- **Курьер (iOS)** ведёт смену, получает список текущих заказов и отправляет геолокацию (**significant location** без активного заказа, **высокая точность** при наличии текущих заказов; точки кэшируются локально и уходят **пачкой** `POST /couriers/{id}/location/batch` при появлении сети).
...
- `docs/COURIER_OFFLINE_GEO_PRODUCT.md` — **продуктовая и архитектурная позиция**: offline-first, Москва/сеть/GPS, UX, связка с кодом (удобно для тестового собеседования).
...
После **«Начать смену»** приложение запросит при необходимости доступ **«Всегда»** и фоновый режим **Location** — так iOS может доставлять **significant location changes** в фоне (как в типичных курьерских приложениях). Без «Всегда» экономичный режим работает в основном пока приложение на экране.
